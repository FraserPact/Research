// Just Trueclassic homepage
const { chromium, devices } = require("playwright");
const fs = require("fs-extra");
const path = require("path");

const BRANDS = [{ name: "Trueclassic", url: "https://www.trueclassic.com/" }];
...
(async () => {
  ...
  for (const { name, url } of BRANDS) {
    console.log(`→ ${name}`);
    try {
      await capture(browser, DESKTOP, name, url, path.join(outputDir, `Homepage - ${name} - Desktop.png`));
      console.log(`  ✓ Desktop`);
      await capture(browser, MOBILE, name, url, path.join(outputDir, `Homepage - ${name} - Mobile.png`));
      console.log(`  ✓ Mobile`);
