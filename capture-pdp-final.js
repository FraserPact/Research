// Final PDP brands: Nike, Onrunning, Outdoorvoices, Quince, Skims, Trueclassic, Comfrt
...
(async () => {
  ...
  for (const { name, url } of BRANDS) {
    console.log(`→ ${name}`);
    try {
      await capture(browser, DESKTOP, name, url, path.join(outputDir, `PDP - ${name} - Desktop.png`));
      console.log(`  ✓ Desktop`);
      await capture(browser, MOBILE, name, url, path.join(outputDir, `PDP - ${name} - Mobile.png`));
      console.log(`  ✓ Mobile`);
